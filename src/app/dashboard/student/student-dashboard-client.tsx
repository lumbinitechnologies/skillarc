...
export default function StudentPage({
  ...
  const stats = [
    ...
    },
    {
      ...
      accent: "bg-[var(--primary)]/[0.04] text-[var(--primary)] border-[var(--primary)]/10",
      icon: <BookOpen size={18} className="text-[var(--primary)]" />,
    },
    {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 pb-8 pt-6 sm:px-6 lg:px-8 space-y-6">
      <div className="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-sm relative overflow-hidden">
        {/* Soft radial aura */}
        <div className="absolute right-0 top-0 w-80 h-80 bg-radial-gradient from-[var(--primary)]/[0.03] to-transparent rounded-full -mr-16 -mt-16 pointer-events-none" />
        <div className="relative z-10 flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4">
            <div className="flex h-14 w-14 items-center justify-center rounded-3xl bg-gradient-to-br from-[var(--primary)] to-[var(--accent)] text-white shadow-lg shadow-[var(--primary)]/10 flex-shrink-0">
              <GraduationCap size={24} />
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--primary)]">Student dashboard</p>
              <h1 className="mt-2 text-2xl sm:text-3xl font-bold text-slate-900 font-['Plus_Jakarta_Sans'] tracking-tight">Welcome back, {student.name}</h1>
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((item) => (
          <div key={item.label} className="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_2px_8px_rgba(15,23,42,0.015)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_45px_rgba(22,144,199,0.06)] hover:border-[var(--primary)]/20">
            <div className={`inline-flex h-10 w-10 items-center justify-center rounded-2xl ${item.accent} border`}>
              {item.icon}
            </div>
      ...
      <div className="grid gap-6 grid-cols-1 lg:grid-cols-[1.15fr_0.85fr]">
        <section className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-[0_2px_8px_rgba(15,23,42,0.015)]">
          <div className="flex items-center justify-between gap-4 pb-4">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--primary)]">Today’s classes</p>
              <h2 className="text-xl sm:text-2xl font-bold font-['Plus_Jakarta_Sans'] tracking-tight text-slate-900 mt-1">Your schedule</h2>
            </div>
            <span className="bg-[var(--primary)]/5 border border-[var(--primary)]/15 text-[var(--primary)] text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md">Live</span>
          </div>
          <div className="space-y-4">
            {schedule.length === 0 ? (
              ...
              schedule.map((item) => (
                <div key={`${item.day}-${item.period}`} className="rounded-3xl border border-slate-200 bg-slate-50 p-4 transition hover:border-[var(--primary)]/30">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-2 text-xs font-semibold text-slate-600 shadow-sm self-start sm:self-auto">
        ...
        <aside className="space-y-6 bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-[0_2px_8px_rgba(15,23,42,0.015)]">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--primary)]">Attendance</p>
            <h2 className="text-xl sm:text-2xl font-bold font-['Plus_Jakarta_Sans'] tracking-tight text-slate-900 mt-1">Session summary</h2>
          </div>
          <div className="grid gap-3 grid-cols-3 lg:grid-cols-1">
            <div className="rounded-3xl bg-[#00C2A8]/5 border border-[#00C2A8]/15 p-4">
              ...
              <p className="mt-2 text-2xl sm:text-3xl font-bold font-['Space_Grotesk'] text-[#00C2A8]">{attendance.present}</p>
            </div>
            <div className="rounded-3xl bg-[#F04438]/5 border border-[#F04438]/15 p-4">
              ...
              <p className="mt-2 text-2xl sm:text-3xl font-bold font-['Space_Grotesk'] text-[#F04438]">{attendance.absent}</p>
            </div>
            <div className="rounded-3xl bg-[#FFB020]/5 border border-[#FFB020]/15 p-4">
              ...
              <p className="mt-2 text-2xl sm:text-3xl font-bold font-['Space_Grotesk'] text-[#FFB020]">{attendance.late}</p>
            </div>
          </div>
            ...
            <div className="mt-4 grid gap-3">
              {[
                ...
                <div key={label} className="rounded-2xl border border-slate-100/60 bg-white p-4 shadow-[0_2px_8px_rgba(15,23,42,0.01)]">
                  ...
                  <p className="mt-1 text-xs font-bold text-slate-800 break-all">{value || "—"}</p>
                </div>
              ))}
            </div>
          </div>
