...
const StudentReportCardCharts = dynamic(
  () => import("./student-report-card-charts"),
  {
    ssr: false,
    loading: () => (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white border border-slate-200 p-6 rounded-3xl h-72 animate-pulse" />
        <div className="bg-white border border-slate-200 p-6 rounded-3xl h-72 animate-pulse" />
      </div>
    ),
  }
...
export function StudentReportCardClient({
  ...
  return (
    <div className="max-w-5xl w-full mx-auto p-8 space-y-8 font-sans text-left">
      ...
      {/* Recharts analysis (dynamically loaded) */}
      {allGraded.length > 0 && (
        <StudentReportCardCharts summaryBar={summaryBar} radarData={radarData} />
      )}
