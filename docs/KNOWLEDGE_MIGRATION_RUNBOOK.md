...
4. Deploy the Next.js producer and worker. `vercel.json` schedules the worker every five minutes; Cron requests must still pass the secret check. Verify Node runtime, memory, outbound model download, cold-start time, and the 300-second function limit.
