...
def build_prompt(question: str, chunks: List[Dict[str, Any]]) -> str:
    """Assemble the full prompt sent to the model: system identity/rules,
    retrieved document context, and the user's question."""
    user_settings = get_settings().get("user", {})
    user_name = user_settings.get("name", "User")
    user_role = user_settings.get("role", "Student")
...
def generate_answer(question: str, chunks: List[Dict[str, Any]]) -> str:
    ...
    try:
        ...
        logger.error("Groq API error during generate_answer: %s", e)
        return FALLBACK_ANSWER
    except Exception as e:
        logger.exception("Unexpected error during generate_answer: %s", e)
        return FALLBACK_ANSWER
...
def generate_answer_stream(question: str, chunks: List[Dict[str, Any]]):
    ...
    try:
        ...
        logger.exception("Unexpected error starting stream: %s", e)
        yield FALLBACK_ANSWER
        return
    ...
    try:
        ...
        logger.exception("Error while consuming Groq stream: %s", e)
        yield f"\n\n_{FALLBACK_ANSWER}_"